...
create table public.record_platform_domain_identity (
  ...
  domain_kind text not null check (domain_kind = 'deletion_witness'),
...
create table if not exists public.deletion_witness_entries (
  ...
  constraint deletion_witness_entries_contract_fields_valid check (
...
create table if not exists public.deletion_witness_head (
...
insert into public.deletion_witness_head(singleton)
values (true)
on conflict (singleton) do nothing;

create table if not exists public.recovery_trust_witness_entries (
...
create table if not exists public.recovery_trust_witness_head (
  singleton boolean primary key default true check (singleton),
...
insert into public.recovery_trust_witness_head(singleton)
values (true)
on conflict (singleton) do nothing;

create trigger deletion_witness_entries_immutable
before update or delete or truncate on public.deletion_witness_entries
for each statement execute function record_platform_internal.reject_immutable_mutation();

create trigger recovery_trust_witness_entries_immutable
before update or delete or truncate on public.recovery_trust_witness_entries
for each statement execute function record_platform_internal.reject_immutable_mutation();
