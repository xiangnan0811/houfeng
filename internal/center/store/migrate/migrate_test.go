...
func TestFrozenR1RootSourcesRemainExactPrefix(t *testing.T) {
	...
	if got, want := names[frozenR1RootSourceCount+9], "0061_create_subscription_create_idempotency.sql"; got != want {
		t.Fatalf("tenth current extension migration = %q, want %q", got, want)
	}
	if got, want := names[frozenR1RootSourceCount+10], "0062_create_vps_create_idempotency.sql"; got != want {
		t.Fatalf("eleventh current extension migration = %q, want %q", got, want)
	}
	if got, want := names[frozenR1RootSourceCount+11], "0063_tune_heartbeat_incident_policy.sql"; got != want {
		t.Fatalf("twelfth current extension migration = %q, want %q", got, want)
	}

	if got, want := names[frozenR1RootSourceCount+12], "0064_add_network_rates_valid.sql"; got != want {
		t.Fatalf("thirteenth current extension migration = %q, want %q", got, want)
	}
	if got, want := names[frozenR1RootSourceCount+13], "0065_extend_vps_lifecycle_audit_and_snapshot.sql"; got != want {
		t.Fatalf("fourteenth current extension migration = %q, want %q", got, want)
	}
