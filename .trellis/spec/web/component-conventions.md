...
- **关联表单优先选择器，不让用户复制内部 ID**：Provider、Node、Target、Service 这类已有业务对象的常规关联表单应渲染可辨识 `<select>`/selector（名称 + ID + 状态/位置等辅助信息），并保留空值/不关联能力。无候选或加载失败时用说明 + `<Link>`/action 指向对应列表/创建入口；Node/Target 选择只作为用户确认的资产关联，不在表单提交时隐式修改观测运行态。
