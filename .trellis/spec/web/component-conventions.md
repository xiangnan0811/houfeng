...
- **资产组合决策页面主体必须决策优先、稳定静默**：`/asset-decisions` 默认页面第一层只回答“当前要不要处理、最该处理什么、下一步做什么”，第二层是“决策组扫描”。历史记录、场景/模板、续费事实和单台队列只能作为紧凑辅助入口，默认不得展开表格、长说明或与主判断同权重的卡片。没有自动组、记录漂移/阻塞/缺证据和局部错误时，顶部返回稳定 lead，不得把场景模板或自定义组 fallback 成主任务，也不得渲染“处理/使用模板/继续组合/打开决策组”类主 CTA。移动端首屏必须能进入“决策组扫描”：低优先级事实条可以隐藏，辅助入口保持 2x2 小入口，只保留短标题、状态和动作。测试要覆盖稳定态不提升模板/手工组合、页面主体无 `PORTFOLIO` / `RENEWAL` / `WORKBENCH` 等英文 eyebrow、辅助入口 accessible name 为 `资产决策辅助入口`，浏览器 sanity 要看 1440px 与 390px 首屏和横向溢出。
- **资产决策详情默认层必须是决策封面，不是压缩报告**：`/asset-decisions` 的自动组、自定义组、保存记录等 modal 默认只回答“对象是谁、当前判断是什么、关键风险是什么、下一步主动作是什么、哪里进入详情”。首屏预算只允许标题/状态、紧凑事实条、单句判断、最多 1-2 个风险/缺口信号、主动作和少量详情入口；成员列表、成员姓名、每成员 `处理` 按钮、保存记录成员快照、执行编排、来源回读、宽表、原始底稿、说明性长句和字段解释都必须收进二级面板。`comparison_insight.summary`、`decision_recommendation.summary`、`goal`、`note`、`execution_readback.summary` 等 API 长文案不得原样进入默认层，必须裁成一条短判断；默认层测试要有可见文本密度上限和“无多句说明”的断言，不能只反向搜旧标题。`查看详情` 的首跳也不能直接展开成员、底稿、保存表单或单台处理；应先进入“详情目录/二级工作台”，再由用户显式选择一个单面板。成员面板使用紧凑决策行，完整字段、宽表和处理动作继续下钻或放入低权重面板。测试要正向断言默认层关键判断与主动作存在，并反向断言成员名、成员摘要、底稿入口、旧解释标题、长报告文案默认不可见；还要覆盖至少一个自动预算/成本组、一个非成本自动组、自定义组合、场景模板和保存记录。浏览器 sanity 要检查桌面与 390px 移动端打开 modal 后 document/body 无横向溢出；表格自身内部横向滚动是允许的。
- **资产决策弹窗必须使用不透底内容面**：资产决策 modal 可以保留暗色 overlay，但 `asset-decision-*` modal 的 content/header/body 必须是不透明主题 surface，不能让底层页面文字透进弹窗任务面板。浏览器 sanity 打开成员、保存、底稿或单台处理面板时，要人工看一张截图或检查 computed background，确认没有页面文本“鬼影”和表单文字重叠。
- **资产决策二级面板必须是单任务短面板**：详情目录只允许 label、count/status、极短 meta；不要在目录项里放完整说明句、成员名、字段解释、内部 ID（如 `adg_` / `admg_` / `adr_` / `adt_`）、来源回读串或宽表线索。进入 `members` / `save` / `execution` / `source` / `create` / `status` 这类二级面板后，同一面板只做一个任务：成员面板默认只展示姓名/角色/动作/一句判断/少量信号/动作按钮，不展示 provider、location、product、成本、facts 串、证据矩阵或英文 eyebrow；保存面板默认只展示基础字段和成员复核行，逐个成员展开编辑；保存记录 `execution` 面板只做状态推进和可执行成员动作，不复用“保存时判断依据”或来源回读；`source` 面板只做来源复核入口，来源详情展示用户可读的来源类型/视图/范围，不展示 `source_group_id` 或后端 group type 机器值；`add` / `create` 面板默认只渲染当前任务的核心字段，可选高级字段必须由用户显式展开，收起或重新进入默认态时要清空隐藏字段 draft，避免提交用户看不到的旧值。测试必须反向断言 `MEMBER DECISIONS`、`SAVE DECISION`、`EXECUTION PLAN`、`SOURCE CONTINUITY`、`CREATE SCENARIO`、`BLUEPRINT`、`保存时判断依据`、provider/product/cost/facts 串、内部 ID 和“推进迁移”这类 marker 不出现在普通二级面板；浏览器 sanity 记录每个典型弹窗在桌面和 390px 移动端的 marker 为空且无 document/body 横向溢出。
- **资产决策成员数组默认必须预览限量**：`members`、保存记录成员复核和保存记录 `execution` 面板遇到多成员数组时，默认只渲染少量预览行（当前实现为 3 条）+ “另有 N 台…”低权重提示；完整成员字段、宽表、隐藏成员编辑和执行细节只能在 `raw` / `members` 等明确底稿或跟进入口继续查看。保存记录提交 payload 仍必须使用完整 draft/member order，不能因为 UI 预览限量丢掉隐藏成员。回归测试要构造 8+ 成员的自动组、自定义组合和保存记录，断言普通面板可见行数不超过预览上限、隐藏成员名不可见、剩余数量提示存在，并继续保留写入 payload 全量成员合同。
