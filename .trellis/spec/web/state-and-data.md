...
- Archive data: `/archive` 显式请求 `listVPSAssets({asset_scope:'archived'})` 和 `listSubscriptions({asset_scope:'archived', sort:'renew_at', order:'asc'})`，按选中 VPS 再读取 `getVPSAsset(vpsId)`、`listVPSServices(vpsId)`、`listVPSDomains(vpsId)`、`getVPSTimeline(vpsId)`。页面只能展示 VPS 基本事实、历史订阅、分币种月成本、服务/域名/Target、监控关联和 timeline，不提供编辑、恢复、取消、创建或关联按钮。多币种订阅摘要必须按币种分开显示（例如 `USD 24.00/月 + EUR 9.00/月`），不得把不同币种相加后套用单一币种。
...
| `/archive` renders detail context | 只读展示 detail、services、domains、timeline 和 monitoring links；不得出现编辑、恢复、取消、添加、创建或关联按钮 |
