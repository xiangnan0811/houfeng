...
- **schema 演进**：`db/migrations/0001_*.sql` … 当前最大 migration（现为 `0021_create_asset_histories.sql`）+ `db/migrations/embed.go` 用 `embed.FS` 嵌入；启动时由 `internal/center/store/migrate/migrate.go` 中的 `Apply` 顺序应用，状态记在 `schema_migrations` 表。
...
2. 在 `db/migrations/` 新建下一个未占用序号的文件，例如当前最大为 `0021_create_asset_histories.sql` 时，下一个应为 `0022_<verb>_<scope>.sql`。
