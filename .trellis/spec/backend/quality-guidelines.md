...
| 新增 VPS timeline / 续费决策历史 | 1) migration 测试证明 `renewal_decisions` 约束、索引、枚举；2) store 测试证明 VPS PATCH 在事务中 `select ... for update`、更新当前决策并按变化插入历史；3) handler 测试覆盖 `/api/vps/{id}/timeline` success、missing VPS、invalid input、method；4) router 测试证明 timeline 不落到 item handler / SPA；5) bootstrap_test 增 nil 断言；6) 验证不改 Node / Target / Agent 状态写路径 |
