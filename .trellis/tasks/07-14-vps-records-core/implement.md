...
### Final review remediation (2026-08-04)

- [x] Remove `project_id` and `record_id` from the durable `record_core_purge_receipts` schema, insert path and digest input. The exact receipt columns are now `operation_id`, `adapter_name`, `removed_surface_digest`, `receipt_digest`, `removed_row_count`, `verified_absent_at` and `created_at`; preview correlation follows the operation/reservation relationship instead of persisting object identity in the receipt.
- [x] Make expired-draft cleanup atomically exclude both `fenced` and `committed` deletion reservations before scan/limit, accept nullable new-record draft IDs, deduplicate existing-record IDs, close scan rows before mutation checks, and recheck each record mutation fence immediately before deletion. Unit and PostgreSQL regressions prove reserved expired drafts/checkpoints survive cleanup.
- [x] Make deletion recovery release only the exact reservation-owned projection tuple (owner, generation and observed expiry), fail closed unless exactly one matching fenced projection is updated, and leave later or unrelated active fences unchanged. PostgreSQL regressions cover terminal replay, idempotent replay and delayed replay after cursor rollback while a newer operation retains its exact fence tuple and continues blocking record reads.
- [x] Re-run the remediation verification matrix: focused source/unit tests; the three-package deletion/draft/recovery race selector for 10 iterations; all 31 real PostgreSQL RecordPlatform/revision/read/draft/deletion/recovery scenarios with zero `SKIP`; records-core schema and fresh/exact-repeat current APP ACL anchors; PostgreSQL 16.12 strict catalog anchors; `make verify-go`; Node 22.23.1 `make verify-web` (126 files, 883 tests, TypeScript/Vite and bundle/CSS budgets); production forbidden scans; `git diff --check`; and Trellis task validation. All are GREEN.
- [x] Complete an independent final re-review of the remediation. Critical: 0, Important: 0, Minor: 0; all three original Important findings are closed. This clears the branch for the commit/CI stage but does not reuse the earlier PR checks as evidence for the new HEAD.
