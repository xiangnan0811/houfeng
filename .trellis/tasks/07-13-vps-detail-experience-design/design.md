...
- independently monotonic `app_acl_manifest_rNNNNNN`，由 `app_acl_manifest_revisions` append-only chain 与 singleton `app_acl_manifest_head` 持久化；每个 revision 绑定按 filename 排序的完整 migration filename/checksum set 与 exact privilege set。初始基线是 0001…0051 的 52 个 SQL 文件，0051 最终含 22 张应用/治理表且不计 runner-owned `schema_migrations`；catalog 验证包含 PUBLIC、membership、ownership、column/default ACL、四条 sequence，并对 APP runtime 精确两项 `public.record_platform_cas_contract_activation_projection(bytea)`、`public.record_platform_cas_domain_rotation_projection(bytea)` 校验 schema、identity args、owner、`prosecdef`、`prokind`、固定 `proconfig`、ACL/default ACL 与零额外 overload；PUBLIC/extension function EXECUTE 必须为 0；
