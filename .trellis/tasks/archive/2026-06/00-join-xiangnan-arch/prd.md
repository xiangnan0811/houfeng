...
`xiangnan-arch` just ran `trellis init` on a fresh clone, saw "Developer
...
- `.trellis/workspace/xiangnan-arch/journal-*.md` — their session log,
...
- Check if `.trellis/workspace/xiangnan-arch/` already exists — if yes, it's
  their journal from another machine and worth mentioning.
- Run `python3 ./.trellis/scripts/task.py list --assignee xiangnan-arch` to
...
python3 ./.trellis/scripts/task.py archive 00-join-xiangnan-arch
```
