...
- [ ] 确认父任务设计仍为批准版本，直接依赖为空，并记录五个 released migration 的 byte-exact SHA-256：APP 0051=`503d58670dc790c4b852bfb58cf93d2b816c1ce956958567dc605cb28d5cd23f`、APP R2 0052=`23f79c60dcede45a42aae82da5a9de0d3d650d7eef64dbfd7ce96c6dd5d95fff`、deletion-ledger 0001=`4138fb2dbd77b6d93fa3c284a3011f7649f01474de62178fbd5562814fcd7505`、deletion-witness 0001=`63a94eded47708e2745ba42fa041782385ed099ed17226bfa2e1236e43f4e11f`、recovery-control 0001=`dd53c54f02c170b97227246ecc1a0f045fb219d03a0e605bcb9baec3cbcc5417`。任何 byte/whitespace 变化立即停止；children 2–11 保留 0052–0060，foundation 只使用 0061–0063 与 isolated `0002+`。
...
- Preserve byte-for-byte, never edit: `db/migrations/0051_create_record_platform_foundation.sql`
- Preserve byte-for-byte, never edit: `db/appaclr2/migrations/0052_app_acl_r2_privileged_transition.sql`
- Create: `db/migrations/0061_upgrade_record_platform_foundation_to_app_acl_v3.sql`
- Create: `db/migrations/0062_add_domain_identity_provision_authority_gate.sql`
- Create: `db/migrations/0063_add_app_acl_v3_readback.sql`
- Create: `db/deletionledger/migrations/embed.go`
- Preserve byte-for-byte, never edit: `db/deletionledger/migrations/0001_create_deletion_ledger.sql`
- Create: `db/deletionledger/migrations/0002_add_record_platform_governance.sql`
- Create: `db/deletionwitness/migrations/embed.go`
- Preserve byte-for-byte, never edit: `db/deletionwitness/migrations/0001_create_full_witness.sql`
- Create: `db/deletionwitness/migrations/0002_add_record_platform_governance.sql`
- Create: `db/recoverycontrol/migrations/embed.go`
- Preserve byte-for-byte, never edit: `db/recoverycontrol/migrations/0001_create_recovery_control.sql`
- Create: `db/recoverycontrol/migrations/0002_add_record_platform_governance.sql`
...
- [ ] 写 migration source/upgrade RED tests：先 hash-check五个 released files并在其任一 byte变化时失败；再从真实 released 0051 + exact R2 `FINALIZED` catalog验证0061–0063和三个isolated 0002的唯一键、CHECK、TTL、无内容列、无cascade、local identity/attestation extension，以及V3 owner/caller/default-ACL合同。运行 `go test ./internal/center/store/migrate -run 'RecordPlatform|AppACLV3' -count=1`，确认因successor缺失 RED。
- [ ] 实现0061 released-schema-to-V3 transformation、0062 provision-authority gate、0063 V3 exact readback，以及三个isolated 0002 additive migrations；不得把旧draft整体复制为0061，也不得修改released files。任何未来 record/object identity均无FK，不提前创建records表。复跑 focused test GREEN。
...
- Modify before first release: `db/recoverycontrol/migrations/0002_add_record_platform_governance.sql`
- Modify before first release: `db/deletionledger/migrations/0002_add_record_platform_governance.sql`
- Modify before first release: `db/deletionwitness/migrations/0002_add_record_platform_governance.sql`
- Modify before first release: `db/migrations/0061_upgrade_record_platform_foundation_to_app_acl_v3.sql`
- Modify before first release: `db/migrations/0062_add_domain_identity_provision_authority_gate.sql`
- Modify before first release: `db/migrations/0063_add_app_acl_v3_readback.sql`
...
- [ ] **Step 2: Add successor-migration RED tests.** Require the exact DDL contract below as final catalog state after immutable 0051/R2 0052 plus 0061–0063 and after each immutable isolated 0001 plus its 0002 successor. Never merge additions back into a released `CREATE TABLE`. Released 0051 remains exactly 22 public application/governance tables: the existing 18 foundation tables, its local domain identity/attestation pair and two APP ACL manifest tables；runner-owned `schema_migrations` and the internal extension schema are excluded from this historical count. The five released files must retain the Preflight hashes. Each physical domain continues to own its local `record_platform_internal` and identity/attestation pair; successor migrations add only local objects/constraints/helpers and never depend on another database's extension namespace or relation. Every immutable table is protected against UPDATE/DELETE/TRUNCATE；the byte bounds shown below are also parser limits. No table contains command lines, local paths, free text, private-key bytes or record/object content.
...
-- This is the required final local shape in every domain. Released 0051/0001
-- bytes stay untouched; 0061–0063 and isolated 0002 successors add only the
-- missing final-state deltas. It is never shared storage.
...
- [ ] **Step 6: Implement additive released-schema successors, stable domain identity, canonical attestation and V3 role grants.** The five released migration files have shipped and must retain their Preflight checksums byte-for-byte. Put APP changes only in 0061–0063 and isolated-domain changes only in their 0002 successor；future changes use the next free additive filename. Install or converge `pgcrypto` only through the declared `schema_owner` phase in `record_platform_internal`, require `pg_extension.extnamespace` to exact-match that schema, and deny PUBLIC/runtime/admin schema access；never silently move an existing extension. `ProvisionDomainIdentity` reads database OID/name, expected kind and preferably `pg_control_system().system_identifier`, generates a random domain ID/member epoch 1, and exact-matches that physical domain thereafter; a new domain may be provisioned only as inactive candidate for Task 17. External mode parses the exact `HOUFENG-RECORD-PLATFORM-DOMAIN-ATTESTATION-POLICY-V1` threshold/key format with the strict 0400 regular/no-follow ≤64 KiB loader and the signature-set `SignedDomainAttestationV1` defined in design §4. Verify magic/version/field count, NFC/bounds, UTC microseconds, purpose, deployment/project/profile/kind/domain/member+set epoch, physical identity fields, stable digest, generation, challenge, validity, policy digest and the sorted unique threshold signatures. Renewal appends only when witnessed policy/stable digest/kind match and generation increases; it never changes identity-set bytes. Candidate policy rotation requires current+candidate thresholds and candidate-only possession. S3 uses the read-control-only principal for TLS/SPKI/bucket/Object-Lock probes and candidate locked-nonce proof; it cannot read witness bodies or PUT normal witness prefixes. Pairwise checks reject alias/system/stable digest and backup/log/workspace/replica overlap.
  ...
  The historical R1/R2 runner has two transaction paths. Fresh genesis applies or adopts the exact 52-file 0001…0051 baseline, then creates r1；later generic pending child migrations use one SERIALIZABLE migration+ledger+ACL+revision+head transaction, so a release containing 0056 before 0055 still advances monotonically when 0055 arrives. Those paths and all R1/R2 rows remain immutable evidence；they do not authorize a V3 caller or permit rewriting an old migration.

  V3 is one separate transition closure, not three independently visible generic migration commits. Acquire the released R2 transition advisory lock on one reserved connection, start `SERIALIZABLE READ WRITE` as the constrained direct migrator, and require shared classification to return exact `FINALIZED` before any write. The short-lived bootstrap provisioner validates a canonical non-secret owner-phase plan, enters `SET LOCAL ROLE schema_owner` only for schema/relation/sequence/type/extension/internal-helper work and `SET LOCAL ROLE definer_owner` only for closed public mutators, applies 0061 then 0062 then 0063, transfers the complete released object set, records all three filename/checksums, converges V3 caller/default/definer ACLs, appends one V3 evidence/head transition and exact-readbacks the complete catalog/data state before commit. Any failpoint rolls back all three migration-ledger rows, data, ownership, ACL and V3 evidence. SQLSTATE 40001/40P01 retries the whole closure；ordinary commit error runs only the default exact V3 ACK observer. Exact V3 returns success, exact released R2 FINALIZED is the only retryable prior state, and every other outcome/error-first condition fails without DDL replay. After commit, revoke/delete the bootstrap identity and require its absence at admission.

  V3 appends to, but never re-encodes, the released R1/R2 chain. Its canonical evidence binds `schema_owner`、`definer_owner`、all operational caller role names、bootstrap plan digest、owner-phase-map digest、managed-object ownership digest、owner-specific default-ACL digests、closed public-mutator identity set and every definer dependency/caller grant. `schema_owner` and `definer_owner` are distinct `NOLOGIN NOINHERIT NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION NOBYPASSRLS` roles with no membership edges；`definer_owner` owns no relation. Runtime、platform-admin、normal migrator、validator and the future trusted projector caller are distinct `LOGIN NOINHERIT` roles, own zero database/schema/relation/sequence/type/extension/function/large-object objects and have no direct/recursive path to either owner. Caller grants and definer dependencies are verified separately；ownership never counts as caller authority.
...
R1/R2 legacy runtime verification is entirely read-only. It requires `current_user` to equal the latest manifest's `center_runtime` binding；requires `schema_migrations` to equal the embedded filename/checksum map；reads all revisions plus head and proves a non-null head, exactly r1…head with no gap/far tail, r1 previous zero, each later previous digest equal to rN-1, and every canonical set/digest/manifest recomputable. Its catalog verifier keeps the released constrained scope and requires the 204-tuple ACL plus empty runtime/admin persistent-function `EXECUTE` sets；the two 0051 projectors remain exact migrator-owned catalog objects with no caller. Pending/unknown/tampered migrations, null/rollback/far-tail head, parser errors and any ACL delta keep records admission unavailable and never invoke migration code. `node_heartbeats_id_seq` intentionally retains its 0001 name after the 0029 table rename.

V3 verification is a distinct complete catalog readback. It requires all three 0061–0063 ledger rows, a continuous released R1/R2→V3 evidence chain, exact owner/caller attributes and membership graph, `datdba=schema_owner`, exact ownership of every managed object, no relation owned by `definer_owner`, no object owned by any operational role, no bootstrap login, exact caller/default/definer ACLs, and one closed `public.name(bytea)` mutator row per V3 manifest identity with the declared owner/return type/security/search-path/dependencies and no overload. A V3 runtime receives only the V3 caller allowlist and accepts this state；the R1/R2 admission classifier must reject it as successor rather than treating extra migrations/functions/owners as repairable drift.

Every V3 `definer_owner` function uses exact `SET search_path=pg_catalog`, schema-qualifies every referenced object, decodes one canonical `bytea` command, validates typed fields plus canonical bytes/digests and expected head/sequence in one transaction, and is `REVOKE ALL ... FROM PUBLIC`; each operational caller receives only its listed fully qualified `EXECUTE`. Identity-set publication is an ordered protocol, not two unrelated inserts: the recovery-control function locks epoch N-1, inserts/exact-matches set N and its typed primary receipt atomically；only that receipt authorizes the full-witness function to insert/exact-match the byte-identical set, a canonical copy of the complete primary receipt plus its digest, and the typed witness receipt that binds that digest. The caller must then read back five physical artifacts before either receipt digest can enter a completion receipt. Candidate-policy and abandon authorization/completion follow the same 2+3 rule. Ack loss retries exact canonical bytes；any mismatch or far tail fails. R1/R2 runtime and platform-admin calls to both 0051 projectors remain `42501`；only a separately manifested V3 trusted projector caller may execute the V3-owned projector identities after exact external witness validation. No operational role receives direct table write, `TEMP`, ownership, DDL, role membership, default privileges or blanket grants. Real-role tests prove all operational roles fail direct registry DML/DDL/`FOR UPDATE`/`SET ROLE`, R1/R2 projector execution stays zero, the V3 trusted caller alone succeeds on exact witnessed projection commands, and every wrong canonical/head/epoch or extra authority fails.

All four abandon artifact tables and all stable identity/attestation tables retain explicit statement-level UPDATE/DELETE/TRUNCATE rejection triggers in addition to ACL denial. Mutation plan/authorization/bundle/completion and approval-detail rows remain insert-or-exact-match immutable；attempts are append-only except a narrow one-time `finished_at` transition. `public.record_platform_compact_completed_mutation_details(bytea)` still decodes `mutation_id`、`expected_plan_digest` and `observed_at`, locks the root, requires state complete、an exact immutable completion receipt、`observed_at >= completed_at+30d`、kind-specific ledger/replay/inventory receipts and zero active workspace, then only deletes attempt/parsed-approval detail and clears `last_error_code`; it never deletes canonical plan/authorization/bundle/trust/ledger/witness/completion bytes. V3 ownership transfer and caller separation do not relax any of these immutable/retention contracts.
...
- [ ] **Step 7: Verify GREEN for both exact profiles.** Add golden/unit selectors `TestAppACLCanonicalMigrationSetBodyV1`, `TestAppACLCanonicalPrivilegeSetBodyV1` and `TestAppACLManifestPersistedV1`, rejecting duplicate/noncanonical entries, bad filename/checksum, oversize, trailing bytes, unknown enum/version and sibling digest mismatch against the PostgreSQL CHECK. Real PostgreSQL selectors are `TestPostgresIntegrationAppACLManifestFresh` (22 0051 tables, 52 migration entries, r1/head, zero previous digest, SELECT-only manifest access and login/VPS/monitoring/IP/subscription/records smoke), `...UpgradeFromV059` (51 name-only rows, checksum/timestamp-safe adoption, both flags-off-before-0051 and flags-off-after-0051 paths), `...RepeatIsReadOnly`, `...TransactionRollback` at all five failpoints, `...RejectsTamper` for ledger/chain/head/catalog/role/PUBLIC/default/column/sequence/function drift, and `...Applies0056Before0055` across two embedded release maps. CLI/bootstrap selectors prove `migrate --scope app` opens only APP migrator and records runtime verifies but never migrates; pending 0052 and an object applied without its manifest revision both fail closed. The `postgres_sync` fixture uses four independent PostgreSQL containers/data dirs and additionally proves ledger/witness/recovery fresh+repeat/checksum/unknown migration behavior, pairwise physical identities, role/function isolation and application backup exclusion. The `s3_worm` fixture uses only app/ledger/recovery PostgreSQL plus locked TLS S3; it proves no witness DSN/role is parsed or opened, provisions S3 stable identity through the read-control principal, rejects expired/rollback/wrong-kind attestations and application-backup overlap, and exact-matches the witnessed identity set. Role tests reject direct immutable/head writes, every cross-entry-kind function call, UPDATE/DELETE/TRUNCATE/ALTER/DISABLE TRIGGER with SQLSTATE `42501` or immutable-trigger SQLSTATE `55000`; allowed function calls commit exact bytes. `pg_dump --schema-only` of the application DB contains none of the independent ledger/witness/recovery tables.
  ...
  Add `TestPostgresIntegrationAppACLV3UpgradeFromExactR2Finalized`, `...RejectsR2PriorAndDriftWithZeroWrites`, `...RollbackAtEveryClosureCutpoint`, `...RecoversLostCommitAcknowledgementWithoutReplay`, `...RoleGraphAndCallerAuthority`, and `...OldAdmissionRejectsSuccessor`. Run the same upgrade corpus against exact `postgres:16.0`、`:16.6` and `:16.12`; assert the three 0061–0063 migration rows become visible only together, all R1/R2 evidence bytes remain unchanged, bootstrap identity count reaches zero after commit, V3 admission succeeds and released R1/R2 admission rejects. Before the two R1/R2 projector call denials and V3 trusted-caller success, authenticate physical pools for every distinct role；test-admin `SET ROLE` or post-hoc ownership transfer is not evidence.
...
- [ ] **Step 2: Add retention RED tests.** Build the eight independent roots in `RetentionRegistryStateV1`; only `CanonicalSchemaRegistryV1` (production bounded encoder/decoder/storage-codec leaves) and manually reviewed `RetentionPolicyRegistryV1` (one class+semantic and concrete stage bindings per exact surface) are semantic classification authorities. `RetentionLifecycleRegistryV1` supplies reusable clock templates, `PurgeParticipantRegistryV1` supplies typed capability rows and executable binding IDs, and the S3/filesystem/client roots supply exact inventory grammars rather than classifications. `go generate` exact-joins all eight roots into checked-in `retention_allowlist_v1.json`. The two immutable initial-program manifests contain the 21/24 canonical rows below, while the live lifecycle/participant roots acquire only the current child's owned rows through the 11 add-only deltas. The generator never invents semantics from field names or database types. Wildcards, relation/schema-only rows, default classification, unknown enum values, a decoder with no policy row, a policy row with no decoder/column/property, missing/extra lifecycle or participant consumer, trailing bytes and any new unclassified column/key/file fail the build and permanent-delete readiness.
  ...
  All policy/claim/acceptance bodies use fixed magic/version/field order, big-endian integers, length-prefixed strings and enum-sorted unique arrays except semantically order-bearing fields, which retain their declared order；`MergeOrder` is exact `1,2,3,4,9,5,6,7,8,10,11` and must never be enum- or numeric-sorted. v1 `GitObjectIDV1` is exactly algorithm `sha1` plus 20 raw bytes. `PolicyDigest=SHA-256("HOUFENG-RETENTION-ACCEPTANCE-POLICY-V1" || canonical-policy-body)`. The production policy constants are exact: `RepositoryID=1232752877`、`TargetRef=refs/heads/main`、merge method `merge_commit`, and five raw-byte-sorted required contexts `docker-image`、`go`、`record-retention/source-claim`、`web`、`web-browser`, each bound to its workflow blob/action-set and `IssuerAppID=15368`. `record-platform-pg16-catalog (postgres:16.0|16.6|16.12)` belongs to a separate ruleset and is invalid inside this five-entry array. The remaining policy fields bind owner-matrix digest、merge order、trusted acceptance and metadata-verifier workflow blobs/action sets、scanner/rules/signer artifact digests、metadata writer/root/App ID、maximum merge-to-acceptance lag and sorted unique acceptance-key history. Exactly one non-compromised acceptance key is active. Key windows are half-open `[ValidFrom,ValidUntil)` UTC-microsecond intervals；`capk-sha256-*` exact-matches public bytes, a signer may create only with the active key at the actual protected-run completion time, retired keys verify only already-created receipts within their original interval, and compromised/unknown/reused key IDs never verify.

  `acceptance-policy.v1` and `genesis-approver-policy.v1` are one atomic authority pair. The loader opens both with no-follow regular-file checks, parses both completely, rejects either missing、`state=provisioning_required`、canonical header plus residual provisioning bytes、digest mismatch or one-sided replacement, and exposes neither until pair validation succeeds. Genesis threshold is exactly 2；every genesis approver ID is `gapk-sha256-<64 lowercase hex>` derived from its Ed25519 public key, and `gak-*`/alias/single-signature inputs fail. Ordinal 1 is the only genesis exception because it introduces the pair, owner matrix and workflows it must subsequently protect. The offline threshold signs `SignedRetentionAcceptanceGenesisV1`, binding exact base/source trees、policy/matrix/tool/workflow digests and a maximum 24-hour expiry；claim 1 and acceptance 1 embed and reverify that complete wrapper. It authorizes only those genesis bytes, not feature behavior or a later policy. After acceptance 1's metadata PR merges, no child 2..11 and no metadata PR may modify the policy, owner matrix, trusted workflows or tool digests. Any later key/tool/workflow/policy change requires a separately approved, non-child `record-retention-policy-rotation` metadata PR whose old-policy threshold authorizes the complete successor and whose receipt is linked before another child starts；that rotation cannot alter the initial merge order or repair a child delta.

  Verification never accepts a caller-supplied `[]RetentionAcceptanceKeyV1` or pre-parsed previous receipt slice. One production decoder verifies the atomic authority pair、policy history、every rotation and previous acceptance from canonical bytes, then returns an opaque `VerifiedRetentionAcceptanceHistoryV1` capability whose constructor is package-private. Scanner、signer report validator、metadata verifier and next-child claim builder require that capability；tests must prove a forged history, skipped predecessor, altered retired window or policy-only partial replacement cannot reach any signing/acceptance API.
  ...
  Credentials are three-way disjoint. The scanner sandbox has a read-only merge tree/report output and no signing key or Git write token. The no-network signer has only the strict regular/no-follow/bounded 0400 Ed25519 key plus distinct inherited report/output FDs, no checkout and no Git/API token. Both CLIs duplicate every accepted inherited FD with `fcntl(F_DUPFD_CLOEXEC, 3)`, verify `FD_CLOEXEC`, own/close only the duplicate and reject reused/same/ambient/stdin-out descriptors；`os.NewFile` ownership of the inherited descriptor is forbidden. After successful signing, the privileged workflow hands back only the exact `retention-signed/` directory and `acceptance.v1` file to the runner UID/GID, requires directory/file modes `0700/0600`, exact-one non-symlink regular file and an actual host `dd` read；the 0400 key is never chowned and is removed by EXIT trap. The metadata writer has only the policy-bound PR App token and signed file bytes, no key and no scanner checkout. The metadata verifier runs the pinned trusted-base workflow/action set, fetches the proposed file as bounded data without executing branch code, and has neither signing key nor write token. Cross-possession, unexpected filesystem/network access, wrong App/workflow/action digest、authority-pair drift or policy-file change fails before signing/writing.
...
- [ ] **Step 3: Add retention acceptance supply-chain RED tests.** Freeze canonical goldens for repository `1232752877`, issuer App `15368`, exact raw-byte-sorted five-context list, separate PG16-ruleset rejection, merge order, genesis threshold 2/`gapk-sha256-*`, the atomic authority pair, opaque verified history, the 11-entry owner matrix, source claim, highest-attempt check snapshot, signed merge acceptance and metadata file. A claim generated twice for the same exact base/source tree is byte-identical and opens no private-key path. Initial child fixtures all reject `no_new_surface`, empty delta and a surface outside their owner entry. Merge acceptance fixtures require a two-parent merge commit with first parent=claim base and second parent=claim source；reject squash/rebase/octopus commits, wrong repository/ref/PR, source/base/tree drift, merge-resolution changes to owned production inputs, stale/non-highest/newer-nonsuccess/wrong-workflow check runs, scanner/rules/signer/workflow digest drift, registry replacement/removal or `before+delta!=after`, caller-injected history, wrong/expired/retired-for-signing/compromised key, cross-repository/child/ordinal replay, duplicate merge SHA, previous-receipt gap and nondeterministic timestamps. Both CLI parsers accept only distinct inherited FDs duplicated with `F_DUPFD_CLOEXEC`; tests assert minimum 3, `FD_CLOEXEC`, no inherited-descriptor ownership, no network and no merge-tree access by signer. Metadata validation permits exactly one non-symlink regular blob at the signed component path and rejects nested/empty/uppercase/underscore/bad-ordinal/slug/short-or-noncanonical-SHA forms. Workflow tests first RED on missing exact runner handback, then require only `retention-signed/acceptance.v1` chown、0700/0600、owner exact-match and host `dd` read while the 0400 key remains root-owned and trap-cleaned. Task 11 fixtures accept only checked-in 1–10 chain plus its own claim；its pre-merge acceptance count remains zero.
...
- [ ] **Step 6: Implement runtime, workers, retention supply chain and janitor contracts.** Center never calls activation/trust/rotation mutation writers and never opens admin/migrator pools. Runtime verifier requires witnessed trust + activation bundle + latest active identity epoch + signed inventory + exact replay watermark. Bootstrap explicitly constructs and registers real `OutboxWorker`, `LedgerReconciler`, `FenceProjector` and `RecoveryInventoryWorker` instances with production stores/adapters; tests drive an actual persisted outbox claim, not-committed release, fence projection and inventory publication through those workers and reject nil/no-op seams or assertions based only on worker count. Implement the independent canonical-schema and manually reviewed retention-policy registries, generate the one-leaf/property manifest, and compare it bidirectionally with every PostgreSQL catalog, S3 object/control-plane property and managed-filesystem inventory at build time and protected-capability startup. The same gate verifies versioned core-dump exclusion evidence at process, systemd, container, kernel/helper and destination layers; an unprovable destination must first become a registered managed surface. Implement the per-family 24h/30d conversion functions with exact state/proof/deadline predicates, live-owner fencing, receipt-bound filesystem/S3 janitors and forbidden-corpus residue scan; no generic “terminal” helper may erase unresolved data.
  ...
  Implement `houfeng-retention-scanner` and `houfeng-retention-acceptance-signer` as separate static tools. Before the foundation PR can merge, a protected tool-build workflow builds them with fixed Go/toolchain/flags, publishes immutable OCI artifacts and records their manifest digests in the reviewed acceptance policy；PR and protected-main workflows download by digest and verify bytes before exec. One atomic `verify-authorities` gate opens and validates both policy files before either source or merge workflow proceeds；acceptance-only、genesis-only and canonical-header-with-residual-provisioning cases all exit nonzero. `record-retention-source-claim.yml` runs for ordinary child PR/merge-queue commits, asserts repository/App/five-context authority and uploads one bounded claim artifact；it never receives the acceptance environment. `record-retention-merge-acceptance.yml` runs only for a just-pushed two-parent feature merge on protected main, verifies the trusted workflow blob/atomic policy pair before requesting the protected environment, runs the scanner read-only, and gives the 0400 key only to a no-network signer sandbox via distinct inherited report/output FDs. After signing, privileged code validates exact output then chowns only the output directory/file back to the runner and proves an actual host read；the key is removed by trap without handback. It then creates or exact-matches the feature-SHA-derived metadata branch/PR using the policy-bound writer identity. `record-retention-acceptance-metadata.yml` runs only for that exact path-only PR, validates the full opaque-history chain without any private key and never emits a child claim. CODEOWNERS and branch rules require exactly `record-retention/source-claim` plus the four existing feature contexts；the three PG16 checks remain separately enforced. Merge method is merge-commit only. Build `/app/houfeng-record-platform-admin` into the image as an alternate entrypoint while the normal image command remains the non-root center；the two CI-only retention tools are release artifacts, not production image entrypoints. Add `make build-record-platform-admin`, `make build-retention-scanner` and `make build-retention-acceptance-signer`.

  Credential/process tests prove the scanner has only a read-only tree and duplicated CLOEXEC report/output FDs, the signer has no checkout/network/Git or PR token and only the 0400 key plus duplicated CLOEXEC report/output FDs, and the metadata writer has only its PR App token plus signed bytes and no scanner checkout/key. The metadata verifier executes policy-pinned trusted-base code, treats the proposed file as bounded data and has neither key nor write token. Ordinal 1 additionally verifies the threshold-2 `SignedRetentionAcceptanceGenesisV1` under `gapk-sha256-*` keys；after acceptance 1 metadata merges, owner matrix/policy/tool/workflow digests are frozen. Any later acceptance-key/tool/workflow/policy change is a separate old-policy-threshold-authorized `record-retention-policy-rotation` metadata PR that must merge before another child starts and cannot alter the initial merge order or repair a child delta. No production function accepts unverified history as a caller-provided slice.
...
2. **released-R2 baseline:** stop only old center and use the released `v0.60.0` image/workflow to apply the immutable 0051 bytes, create exact R1, then bootstrap/finalize the released APP R2 transition to exact `FINALIZED`. Preserve the existing `current-flags-off` and `flags-off-old-rollback` receipts around the 0051 pre-R2 cutpoint, then emit `released-r2-finalized` with all released file hashes、R1/R2 receipt/M2/source/ACL evidence and login/VPS/monitoring/IP/subscription smoke.
3. **pre-V3 rollback:** before any 0061–0063 write, restart the released v0.60.0 binary and prove exact R2 remains serviceable；emit `pre-v3-r2-rollback`. Stop every old center/worker before the V3 transition.
4. **V3 upgrade:** run only the explicit APP V3 bootstrap command with constrained direct-migrator and short-lived bootstrap-provisioner inputs. It must take the released R2 advisory lock, classify exact FINALIZED, and apply 0061–0063/data/owner phases/V3 evidence/readback in one SERIALIZABLE closure. Inject failure after each migration ledger row、DDL/data step、owner phase、ACL、head and readback and prove exact R2 rollback；inject one ordinary lost commit ACK and prove exact V3 recovery with zero DDL replay. Emit `v3-upgrade-retry`, revoke the provisioner and prove no bootstrap credential/name remains.
5. **current-V3-records-only:** start current center with only the V3 runtime DSN, `HOUFENG_RECORDS_ENABLED=true` and permanent-delete false. Prove V3 admission accepts the full history/catalog, released R1/R2 admission rejects the successor, all operational roles own nothing/cannot `SET ROLE`, and only the separately manifested V3 trusted projector caller can execute exact projector commands. No external-domain/mode/keyring input is touched. A later root child filename such as 0052 may be added below 0061 only through the normal V3 migration+manifest transaction；object-only or manifest-only partial state fails closed, and the 0056-before-0055 sequence remains monotonic. Emit `current-v3-records-only`.
6. **activation:** run `migrate --scope permanent-delete` for both `postgres_sync` and `s3_worm` fixtures, prove exact profile identities/ACLs, then plan/sign/apply sequence 1. Inject one ack-loss, delete all local plan/approval files and recovery-control state, reconstruct `status` from full witness by mutation ID, produce a same-scope continuation if the original receipt expires, resume by mutation ID, wait for projection/replay/inventory and emit `activation-retry` only after bodyless exact-204 admission.
7. **post-activation fence:** start v0.59.0 and released v0.60.0 without making either a target. Their legacy readiness may respond, but neither produces V3 membership/queue receipts or receives protected traffic/tasks. Emit `post-activation-old-fenced`; no down migration or old-binary rollback is attempted after V3 commit or activation.
...
- 五个 released migrations 永不改写或 down；0061–0063/isolated 0002 在首次 commit 前任一失败整事务回到 exact R2/0001 baseline。V3 commit 后不回退到 R1/R2，也不删除 successor migration；只允许在同一 V3 contract 上 forward repair，feature/capability 保持 fail closed。
