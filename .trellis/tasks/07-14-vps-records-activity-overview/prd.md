...
本任务拥有 `0057_create_record_activity.sql`，只支持 fresh/current development database 与 exact repeat。`0057` 必须在同一 PR 登记 current APP ACL fragment/admission tests。`experience_logs` 不注册 activity source、不转换；旧 VPS 页面只可作为开发期 feature-off 回滚，不是 staging/legacy compatibility gate。
...
- 使用 `db/migrations/0057_create_record_activity.sql`。本任务启动时 `0051–0056` 已在 protected main；如 `0057` 被任务外占用，先回到父任务统一重划全部尚未实施编号，不在本任务局部制造兼容顺序。
...
- event-time 规则固定为：新记录/revision 1 使用用户确认的 `occurred_at` 且只生成一个创建事件；后续正式 revision/状态/可见范围变化使用本次 `saved_at`；晚到系统事实保留原发生时间并标 `backfilled`；系统事实使用来源权威发生时间；证据使用实际观测终点并另显捕获时间；评论/行动项使用自身不可变变更时间。纠正通过新 correction event 引用旧事件。
...
- `records_v2_read` capability/feature flag 可用于增量开发和 Child 11 集成验证；旧页面在新能力未闭合时可作为 feature-off shell，但不要求 staging 对照、旧数据迁移或 legacy 双写。回退只关闭新 route/capability，不删除 Records 数据。
...
- [ ] `0057` fresh/repeat migration、current APP ACL convergence/runtime admission 均通过；projection 表、subject relation、source unique key、ingest sequence、checkpoint、查询索引和无 source cascade 约束与设计一致。
- [ ] 新记录/revision 1、后续 revision、状态/visibility、late/backfilled系统事实、资产事实、监控事件、证据、评论和行动项逐类 event-time golden tests 通过；同一 source event/version 在retry和从空投影rebuild后的activity ID/业务全序一致、重复数为0；灾难重建递增generation，旧cursor稳定过期并给同query恢复入口，correction不改写旧行。
...
- [ ] feature flag off/on 的当前页面与新页面开发回滚路径可测；关闭 flag 不删除或改写新 projection/records/evidence，也不要求旧 experience 数据对账。
