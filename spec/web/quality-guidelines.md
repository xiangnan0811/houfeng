...
跨前后端改动（同时动了 `internal/` / `agent/` 与 `web/`）使用 `./scripts/verify.sh` 一把跑完，等价于 `make verify-go && make verify-web`（参考 `spec/backend/quality-guidelines.md` 的"命令门户"段）。
...
- ❌ 任何 codegen（前端类型与 Go contract 手对齐，详见 `spec/web/state-and-data.md`）
...
- 测试与被测**同目录、同名 + `.test.tsx` / `.test.ts`**（如 `Button.tsx` ↔ `Button.test.tsx`）。详见 `spec/web/directory-structure.md`。
- **不要**建集中 `__tests__/` 目录。
...
| 改动 | 必须连带的修改 |
|------|----------------|
| 新增 / 修改 center HTTP 端点的请求 / 响应字段 | 1) 后端按 `spec/backend/` 改完；2) `web/src/lib/types.ts` 加 / 改 `*Record` `*Input`，**保持 snake_case 与 Go JSON tag 一致**；3) owning `web/src/lib/*Api.ts` façade 加 / 改函数；4) page / component 调用方更新；5) 必要时 page 测试的 `toHaveBeenLastCalledWith` 断言一起更新 |
...
| 新增 / 改 CSS 令牌 | `web/src/styles/tokens.css` 同步检查 3 套运行时主题（`:root` / `theme-houfeng-light` / `theme-classic-dark`）；`classic-light` 复用 `houfeng-light`，见 `spec/web/styling-guidelines.md` |
...
> 用于后续代码评审；若形成可复用规则，更新 `spec/` 或 `docs/design/current/`。
