...
顶层职责划分（当前产品与界面设计见 `docs/design/current/`）：
...
**生产部署**：`npm run build` 产物在 `web/dist/`，由 center 通过 `HOUFENG_WEB_DIST_DIR` + `internal/center/http/handlers/spa.go` 兜底吐给浏览器（详见 `docs/deploy/local-and-systemd.md` 与 `spec/backend/directory-structure.md`）。
