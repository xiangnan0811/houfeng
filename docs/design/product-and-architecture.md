...
下图仅说明浏览器、center、数据库与 outbound agent 的通信关系，不是完整部署清单。Records authority、附件处理及 Compose 服务拓扑见 [生产部署合同](../spec/contracts/platform/production-compose.md) 与 [部署入口](../deploy/README.md)。
...
- Subscription creation protects billing records from duplicate submissions; its request key, error and receipt lifecycle rules live only in the [subscription contract](../spec/contracts/subscriptions.md).
