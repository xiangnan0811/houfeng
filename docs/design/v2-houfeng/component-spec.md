...
2. **关键状态指标** 内联在状态栏 metadata 下方，最多 4 个紧凑 Link metric，不再作为独立 Dashboard summary strip，也不是 5 列大型 KPI card strip，不展示 Sparkline。异常态优先显示异常对象、严重、`24h 变化`、维护；正常 / 维护态优先显示节点、目标、`24h 变化`、通知或维护。每个 metric 保留 PR4 filtered URL：异常节点 `/nodes?abnormal=1`、异常目标 `/targets?abnormal=1`、严重 `/events?severity=严重`、维护 `/events?maintenance_only=1`、`24h 变化` `/events?time_range=24h`、节点 / 目标管理入口按库存状态优先跳转。首次接入态不渲染关键状态指标。
