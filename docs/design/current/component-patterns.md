...
- Service Modal rows keep fixed identity/status/type, full-width entry with copy, association facts, and optional note groups. Short values do not move subsequent groups; narrow screens stack predictably. Do not repeat a field label in its missing-value text.
