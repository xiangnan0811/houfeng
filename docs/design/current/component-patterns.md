...
- Service/domain Modal rows balance a light resource surface with compact engineering facts: muted border/fill, small spacing, no nested cards or shadows. Reuse the page resource name/ID/status hierarchy; keep ordinary labelled facts compact and wrapping rather than a rigid label-column form. Full entries, labels and notes retain their own line; notes are subordinate. Preserve adjacent entry copying, all association/billing facts and status semantics. Resource-only surface rules do not apply to monitoring rows or the accepted non-modal lists.
