...
- Service/domain Modal rows put the resource name beside existing type/state Badges and the quiet record ID below it. Use stable labelled fact columns on desktop and one column at narrow widths; entry, labels and notes keep full width and wrapping values. Retain adjacent entry copying, all association/billing facts and existing status semantics. The resource grid does not apply to monitoring association rows.
