...
- Match width to content (decision 380px, facts 980px, subscription and object reading 780px), with narrow-screen gutters. Structured forms use task-oriented sections, wide names/hosts, short ports and adjacent switches; keep readable controls, fixed header/actions and an independently scrolling body. Validation and write feedback remain visible beside fixed actions. Subscription facts here are a new record, not an edit: retain amount/currency, period length/unit, dates, all renewal modes and custom payment/currency behavior.
- Monitoring, service and domain dialogs share one count and a name/status/auxiliary-ID object structure, separated by modest rules rather than nested cards. Keep stable labelled facts, full entries with adjacent copy, and genuine details routes only. Distinguish monitoring configuration from observed health and label missing facts. Domains may enrich an association ID with a service name from the scoped service API; optional metadata failure must not block domain reading or reuse another VPS’s data. Preserve all association/billing facts, read-only, write ownership, duplicate-link and existing unsaved-close/focus/scroll safeguards.
...
The shared editor groups existing fields into identity/provider, location, connection/host, and usage/note sections. Reuse the VPS-detail structured-form template with readable headings, two desktop columns and one column on narrow screens. Keep the header and feedback/save/cancel footer fixed while the form body scrolls independently. Both overview and legacy detail use this editor without changing provider snapshots, country/IPv6/SSH field behavior, version-conflict recovery, or write ownership. It is not the VPS creation flow or a lifecycle decision surface.
