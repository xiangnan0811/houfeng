...
- Associated monitoring, service and domain reading stays in the existing Modal, with one 已关联… title, one count and compact separated rows, not nested panels/cards. Use full monitoring names/IDs, distinguish 监控配置 from 观测健康, and label missing provider/location/heartbeat facts. Service-to-Target links use 入口探测 terminology without changing the association. Preserve read-only, write-pending, duplicate-link and focus/scroll safeguards.
- Service/domain Modal rows balance a light resource surface with compact engineering facts: muted border/fill, small spacing, no nested cards, shadows or whole-row hover. Reuse the page name/ID/status hierarchy and monospace domain identity. Arrange ordinary labelled facts in two stable columns with wrapping values, not an inline sentence or a rigid label-gutter form. Full entries, labels and notes span both columns; notes remain subordinate. Preserve adjacent entry copying, all association/billing facts and status semantics. Resource-only rules do not apply to monitoring rows or accepted non-modal lists.
