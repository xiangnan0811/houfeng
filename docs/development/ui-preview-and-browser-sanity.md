...
>
> Design guidance: `docs/design/interface-language.md` and `docs/design/component-patterns.md`.
>
...
预算策略、coverage/Chromium/staging 的强制门禁与例外统一见 [Web 质量规范](../spec/web/quality-guidelines.md#scenario-coveragechromium-与-staging-审计门)。本页仅说明如何运行和记录浏览器验证。
...
已知实现缺口：`scripts/visual_evidence.py` 的 `observability-support` mock 尚未提供 Monitoring 页面消费的 `/api/monitoring-instances/runtime-summaries`，因此这一 helper 不能单独证明当前 Monitoring 正常数据视图；使用仓库 Chromium fixture 或真实本地 Center 补证，并在结果中注明缺口。本次文档整合不修改 helper 业务行为。
