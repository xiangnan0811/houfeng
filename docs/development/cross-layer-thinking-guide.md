...
最小权限数据库边界还要区分“catalog grant 正确”和“生产 SQL 可执行”：`ON CONFLICT` target、`RETURNING`、row lock、trigger 或函数都可能引入隐式权限。只要 SQL 依赖 least-privilege runtime role，就应使用 production repository + direct runtime session 执行代表性 DML；fake transaction 与 catalog-only 断言不能替代这项证据。具体合同由 [数据库规范](../spec/backend/database-guidelines.md)及其链接的平台合同定义。
...
**Bad**: Duplicated validation rules drift, or one layer assumes an untrusted caller has validated its input.

**Good**: Give each trust boundary explicit validation responsibilities and reuse shared rules where appropriate. Retain database constraints as the final integrity boundary; HTTP validation does not replace service authorization or database integrity.
