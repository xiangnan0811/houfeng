...
1. `docs/design/interface-language.md` —— UI tone、视觉默认、状态语言、证据语言与浏览器 sanity 边界。
2. `docs/design/component-patterns.md` —— 当前组件默认、页面职责、测试期望与历史参考边界。
3. `docs/design/product-and-architecture.md` —— 产品形态、拓扑、领域模型与安全边界。

普通 UI work 从当前授权、[规范](../README.md) 和 [设计](../../design/README.md) 出发；历史通过 Git 追溯，不冻结当前产品方向。

**禁止**：

- 不要把早期 concept 屏 / `stitch/` 子目录视觉恢复成当前实现目标——这些是历史素材。
- 不要在历史版本目录里承载新的当前决策；可复用结论应写入 `docs/design/` 或 `docs/spec/`。
- 预览、浏览器 sanity 与本地截图政策见 `docs/development/ui-preview-and-browser-sanity.md`；不要提交 screenshot manifest 或 bulk raster screenshots。只有用户明确批准的 public README/docs 图片资产才可放入 allowlisted docs asset path。不要恢复旧视觉验证流程或一次性历史截图作为当前 workflow。
