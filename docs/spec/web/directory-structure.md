...
**生产部署**：`npm run build` 产物在 `web/dist/`，由 center 通过 `HOUFENG_WEB_DIST_DIR` + `internal/center/http/handlers/spa.go` 兜底吐给浏览器（详见 `docs/deploy/local-and-systemd.md` 与 `docs/spec/backend/directory-structure.md`）。
...
├── css-budget.json             # CSS AST/source/production 预算（已登记 owner）
...
> 上面的目录树以 `web/src/` 实际内容为准。`components/atoms/` 承载设计系统原子，同级文件承载组合组件（`IncidentList.tsx`、`EventList.tsx` 等）；保留两层职责划分。
...
- ❌ 未登记 owner 或依赖其他路由偶然加载的 CSS：共享入口与已登记的路由样式共同组成加载图，规则见 [样式规范](styling-guidelines.md)。
...
1. **`components/atoms/` 是设计系统原子的稳定落点**（`Button` / `Card` / `Sparkline` / `Mono` / `Hostname` / `Timestamp` 等都在此）。
2. **`app/layout/`、`app/RequireAuth.tsx`、`app/metadata.ts` 是当前应用壳的组织方式**。
3. **`auth-client.ts` 复用 `apiRequest.ts` 的 transport primitives**，401 hook 由 `auth-context.tsx` 绑定；不得新增第二套 fetch 包装。
