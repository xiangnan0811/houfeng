...
候风前端的质量门是一条根目录命令：`make verify-web`。**CI 与本地跑同一套 Makefile target**，`.github/workflows/ci.yml` 的 Web job 运行 `NODE_ENV=production make verify-web`，验证该 target 自行隔离调用者环境；包含工具链检查、安装依赖、lint、coverage、production build、bundle 与 CSS 检查。

跨前后端改动（同时动了 `internal/` / `agent/` 与 `web/`）使用 `./scripts/verify.sh` 一把跑完，等价于 `make verify-docs && make verify-go && make verify-web`（参考 `docs/spec/backend/quality-guidelines.md` 的"命令门户"段）。
...
- ❌ 任何 codegen（前端类型与 Go contract 手对齐，详见 `docs/spec/web/state-and-data.md`）

---

## 命令门户

以下 npm 命令在 `web/` 运行，来源为 `web/package.json` 的 `scripts`；根目录可使用 `npm --prefix web ...`。Makefile 门禁始终在 checkout 根运行。
...
| `npm run test` | `vitest` —— 默认进入 watch；本地单次运行可用 `npm run test -- --run` | 本地迭代入口；CI 与 `make verify-web` 使用 `NODE_ENV=test npm --prefix web run test:coverage`，单次普通测试不能替代 coverage 门禁 |
...
**Makefile 端**（`test-web-toolchain` 和 `verify-web` targets）：
...
| 无真实 staging environment/凭据 | 仓库 gate 可合并，但不得把真实 staging 验收（历史 Gate C）标记完成 |
...
- 测试与被测**同目录、同名 + `.test.tsx` / `.test.ts`**（如 `Button.tsx` ↔ `Button.test.tsx`）。详见 `docs/spec/web/directory-structure.md`。
- **不要**建集中 `__tests__/` 目录。
...
5. [ ] **需要人工视觉判断** → 对照 `docs/design/{interface-language.md,component-patterns.md}`，并按 `docs/development/ui-preview-and-browser-sanity.md` 记录 preview、routes/viewports、正式 browser result 与人工判断。额外本地 helper 证据必须标注 local-only；如果改变可复用 UI 方向，同步更新当前 design/spec。
...
| 改动 | 必须连带的修改 |
|------|----------------|
| 新增 / 修改 center HTTP 端点的请求 / 响应字段 | 1) 后端按 `docs/spec/backend/` 改完；2) `web/src/lib/types.ts` 加 / 改 `*Record` `*Input`，**保持 snake_case 与 Go JSON tag 一致**；3) owning `web/src/lib/*Api.ts` façade 加 / 改函数；4) page / component 调用方更新；5) 必要时 page 测试的 `toHaveBeenLastCalledWith` 断言一起更新 |
...
- ❌ **CI 红了 force-push 改一行试**：在根目录用 `NODE_ENV=production make verify-web` 复现 Web job；Chromium 等其他 job 按各自实际命令复现。找到根因并通过受影响门禁后再提交。
...
- ❌ **新增第三方依赖不评估**：每加一个依赖都要权衡 bundle size 与维护成本；当前依赖清单以 `web/package.json` 为准。
