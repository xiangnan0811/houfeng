...
候风前端组件统一采用 **函数式组件 + React 19 hooks**，TypeScript 严格 props（`tsconfig.app.json` 启用 strict）。当前组件和页面设计先参考 `docs/design/interface-language.md`、`docs/design/component-patterns.md` 与现有 `components/atoms/` / `components/filters/` 实现。早期版本化设计稿、Stitch 截图和旧 redesign 过程材料只作为历史背景；如需改变当前方向，更新 `docs/design/` 和相关测试，而不是回退到旧版本标签。

实读约束（来自 `web/src/components/`、`web/src/pages/`）：

- 全部命名导出（`export function <Name>(...)`）；**未发现任何 `export default`**。
- 组件本体使用 `function`；`useMemo` / `useCallback` 按身份稳定性、计算成本或已测回归需要使用，不为抽象而预先包装所有函数。
...
- **复杂前端变更需要可见证据**：新页面、重要详情区块、dashboard/工作台、复杂数据矩阵、资产决策或深度报告存在信息架构/视觉层级未决选择时，用可在浏览器打开的 mockup 或实现预览展示密度、字段优先级和页面承载方式，再就真正未决的用户选择确认。既定且已授权的设计直接实现，不强制 brainstorming、Superpowers companion、独立设计文档或阶段审批。可按需使用 frontend-design 等工具，临时 mockup 保持本地且不作为项目权威。正式实现仍遵守 tokens、BEM、中文文案及测试合同，并检查真实浏览器的状态、交互和相关尺寸；环境不可用时如实报告视觉验收缺口，文字说明不算浏览器验收通过。
...
- `useCallback` / `useMemo` 已用于 Records 工作区、草稿和监控图表等；引入时说明稳定引用或计算需求，并验证依赖与闭包正确性。
