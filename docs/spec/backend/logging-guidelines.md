...
`agent/runtime.NewWithRuntimeDeps` 的构造器也允许 `nil` 回退到 `slog.Default()`，但生产路径恒走显式 logger。

---

## Log level 约定

通用日志约定使用 `Info`、`Error` 两个 level；下方记录了现有实现偏差。新代码保持同一约定：

| Level | 何时使用 | 例子 |
|-------|----------|------|
| `Info` | 进程 / worker 生命周期事件、首次绑定成功、retention pass 完成、session sweep 删除条数 > 0 | `agent/runtime.Runtime.Run` 的 `"agent runtime started"`；`agent/runtime.Runtime.enroll` 的 `"agent enrolled"`；`retention/worker.go` 的 `"retention pass completed"`；`auth/cleanup.go` 的 `"session sweep"` (deleted > 0) |
| `Error` | 操作失败但 worker 选择继续；HTTP / DB 异常需要排查 | `incidents/service.go` 的 `"evaluate monitoring instance incidents after sync failed"`；`retention/worker.go`；`auth/cleanup.go` 的 `"session sweep"` (with error)；`agent/runtime.Runtime.Run` 的 `"sync queue flush failed"` |
...
`Warn` **保持空缺**，把"该 retry / 后续会被 sweep 兜底的失败"也归到 `Error`，让 systemd / 日志聚合更容易抓异常。

**实现缺口**：`internal/center/subscriptioncosts/reminders.go`（审计时 59、159、169 行）与 `worker.go`（32、42 行）仍调用 `Warn`，与本约定不一致。影响是按 Error 收集的告警可能遗漏这些失败；本次不修改业务代码，后续应统一级别或经明确决策调整合同。
...
| `monitoring_instance_id` | `incidents/service.go` | Center 内部 incident 定位使用的 MonitoringInstance 主键 `mi_xxx`；agent enrollment/sync 日志禁止记录该字段 |
...
| `server_url` | `agent/runtime.Runtime.Run` | agent 当前指向的 center origin，仅保留 parsed scheme + host，不含 userinfo/path/query/fragment |
| `status` / `binding_status` | `agent/runtime.Runtime.enroll` | 仅记录 allowlisted enrollment status 与已验证的 bound binding status，不原样透传未知值 |
...
1. **进程启动 / 关闭**：`Run` 入口 `Info "<svc> started"` + `defer Info "<svc> stopped"`。例：`agent/runtime.Runtime.Run`。
2. **重要状态变更**：agent 首次 enroll 在凭据持久化成功后记录 allowlisted status/binding status（`agent/runtime.Runtime.enroll`），但不记录 MonitoringInstance ID、token 或 fingerprint；其他例子包括 binding 状态变化、retention 完成扫描。
3. **worker tick 内的失败**：worker 必须吞错继续，**每次失败必须 `Error` 一行**，把失败位置 + 允许的结构化上下文 + `error` 一并记录。例：`incidents/service.go`、`retention/worker.go`、`auth/cleanup.go`、`agent/runtime.Runtime.Run`；agent queue 错误只记录稳定 action/reason/status/allowlisted code/count，不能带持久队列或实例标识。
...
- ❌ **敏感字段**：以下字段**禁止出现在 log value 里**：
  - `sync_token` / 任何来自 `enrollment.SyncToken`（`agent/runtime.Runtime.enroll` 故意只 log allowlisted `status` / `binding_status`，不 log `monitoring_instance_id` 或 `sync_token`）
