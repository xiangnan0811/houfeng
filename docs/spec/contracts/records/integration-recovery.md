...
边界：`record_markdown_client` / `record_comparison` 删除适配器仍需满足各自完整 readiness 合同；`houfeng-record-platform-admin` 不是备份 CLI，混版本恢复不受支持。本文件不授权 staging 或发行操作。后续迁移与 authority 演进见 [当前 APP ACL](../platform/app-acl-current.md) 与 [Compose authority](../platform/compose-authority.md)，不能将历史实施范围解释为它们尚未实现。
...
- 相关集成 不加 root migration。`0059` 是后续 portability owner 修复：musl 安全的 `blob_key` CHECK，不改 `0058` 原文。
...
- 通用 PostgreSQL fixture 的每个数据目录使用上限 4 GiB 的临时 tmpfs；上限不预分配内存，container 删除后数据随之消失。完整百万行性能门禁不能借用 capacity profile 的缩放来规避空间不足，运行主机须提供足够内存/交换空间。
- 完整 store/migrate 验证分三类依赖执行，而不是把缺失依赖记作 skip：通用 PostgreSQL lane 排除精确的 `TestPostgresIntegrationAppACLR2` 及六个 S3 场景；R2 交由 `postgres-validation.md` 的三个精确镜像 lane；S3 交由 `run-records-integration.sh --profile s3`，包括四个 `TestPostgresMinIOIntegration…` 场景及 `TestPostgresIntegrationAttachmentDeletionPurgesExclusiveS3BlobExactlyOnce`、`TestPostgresIntegrationAttachmentUploadServiceS3DirectWorkflow`。三类全部 RUN/PASS 才能宣称完整门禁通过，任何一类的 skip/fail 都不能豁免。完整包级运行可使用 `go test -timeout=30m`，不改变场景内部的性能预算。
- S3 profile 的报告 `suites` 与 `config_digest` 包含上述六个确切测试名；成功路径也将 verbose child 运行证据输出到 stderr，stdout 保留 profile JSON。归档 gate 输出时须同时保留两条 stream，确认六项 RUN/PASS 后才可与 PostgreSQL lane 合并验收。
- 冻结 R1 catalog/convergence fixture 只物化 `0001…0051` 的 52 项历史迁移，运行时校验使用相同冻结源集；当前全量迁移仍由 current 合同验证。不能为了让历史 fixture 接受当前 schema 而放宽未知 managed object 拒绝规则。
