...
> 本文维护协作 API façade、受控组件、Inbox lazy route 与 unread eager seam。`RecordWorkspace` 已挂载协作控件；workspace/controller 拥有状态与请求，组件保持受控。
...
不读 route、不拥有 server cache；Records workspace/page 负责组合和挂载。
...
- 四个 collaboration component 保持跨页纯受控；route-private controller/state 留在
  `RecordInboxPage`，Records workspace composition 由 `RecordWorkspace` 拥有。
