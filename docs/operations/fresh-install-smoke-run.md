...
If the installer exits with `houfeng-agent install: minisign is required to verify release checksums`, check whether the generated command contains `--install-missing-deps`. A command without that flag was likely generated by a stale `v0.55.0` center, whose embedded installer cannot recover when `minisign` is absent. Debian 11 bullseye may also report `E: Unable to locate package minisign`, so package-manager installation is not a reliable smoke-run fix. Upgrade the center to `v0.55.1` or newer, restart it, regenerate the MonitoringInstance command, verify the regenerated command includes `--install-missing-deps`, and run the new command against the target host. Keep signed manifest verification enabled; do not use checksum-only or unsigned binary fallbacks.
