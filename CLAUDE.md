...
V1 business structure (data model / rules / technical choices / operation flow) is retained in `docs/design/v1-baseline/` as the factual baseline for current Node, Target, ProbeItem, agent, incident, and notification semantics. Visual authority is `docs/design/v2-houfeng/`.
...
- `http/` — `router.go` builds the routing tree from `RouterOptions`; bootstrap fills every handler explicitly so wiring is grep-able. `http/middleware.go` carries `RequireSession` (cookie-backed auth gate) used by every non-agent / non-health route. Subpackage `http/handlers/` has one file per resource (`nodes`, `targets`, `incidents`, `events`, `dashboard`, `settings`, `agent`, `runtime_facts`, `runtime_controls`, `node_onboarding`, `health`, `spa`, `auth`, `metadata`, Asset Ledger handlers). JSON helpers in `handlers/json.go`.
...
- `targets/`, `nodes/`, `agentplan/`, `runtimefacts/`, `observations/`, `ids/` — core observability domain helpers.
...
- Generated install commands must come from the center via `POST /api/nodes/{node_id}/install-command` and use `HOUFENG_PUBLIC_BASE_URL` as the URL authority.
...
- The generated token is short-lived and one-time. Regeneration invalidates the previous active token for that Node.
