...
本文件是 Houfeng 的项目权威入口；规范在 [docs/spec/](docs/spec/README.md)，现行产品与设计在
[docs/design/](docs/design/README.md)，开发命令见 [CONTRIBUTING.md](CONTRIBUTING.md)。
...
- 允许在已授权范围内使用实现子代理和并行协作，明确文件责任；调查者与审查者只读。
  只读角色限制不限制主控及另行授权实现者的修复。见 [协作与验证](docs/development/agent-collaboration.md)。
...
- 修改前运行 `sh scripts/setup-git-hooks.sh`，保留 `.githooks/` 安全保护，不绕过 hooks。
  提交前与推送前核对实际候选、规范同步、独立审查及适用检查，见 [分支与交付](docs/development/branch-workflow-governance.md)。
...
| 范围 | 权威与入口 |
| --- | --- |
| Go center、agent、PostgreSQL | [后端规范](docs/spec/backend/README.md)，根目录 `make verify-go` |
| React SPA | [Web 规范](docs/spec/web/README.md)，根目录 `make verify-web` |
| 跨层、分支、协作 | [通用指南](docs/development/README.md)，根目录 `make verify` |
| 产品、架构与 UI | [现行设计](docs/design/README.md)；历史版本通过 Git 追溯 |
