...
then
  printf 'docker is required for records integration profiles\n' >&2
  exit 1
fi
...
# fails the child on any `--- SKIP:`. This script keeps that contract.
...
suites = [
    ...
    "store.RecordPortabilityDeletion",
    "store.RecordWatchVersionedDefaultAnchor",
]
if profile == "s3":
