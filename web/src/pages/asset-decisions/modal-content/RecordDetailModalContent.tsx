import { Modal, TabPanel, Tabs, Badge, DataTable } from '../../../components/atoms'
...
import type { RecordDetailPanel, RecordFollowupDraft } from '../types'
import { RecordExecutionBoard } from '../components/RecordExecutionBoard'
import { RecordFollowupRows } from '../components/RecordFollowupRows'
import { createRecordMemberColumns } from '../tableColumns'
...
type RecordDetailModalProps = {
  ...
  onPatchRecordStatus: () => void
  onSetRecordPatchStatus: (status: AssetDecisionRecordStatus) => void
  onOpenRecordSource: (detail: AssetDecisionRecordDetail) => void
  onUpdateFollowupDraft: (vpsID: string, patch: Partial<RecordFollowupDraft>) => void
  onEditFollowupMember: (vpsID: string | null) => void
  onSaveFollowup: (member: AssetDecisionRecordMember, nextStatus?: AssetDecisionFollowupStatus) => void
...
export function RecordDetailModal({
  ...
  onOpenRecordSource,
  onUpdateFollowupDraft,
  onEditFollowupMember,
  onSaveFollowup,
  onReviewRecord,
}: RecordDetailModalProps) {
  const recordMemberColumns = createRecordMemberColumns({
    onRenderFollowupForm: (member) => (
      <RecordFollowupRows
        surface="cell"
        members={[member]}
        drafts={followupDrafts}
        saving={followupSaving}
        editingMemberID={followupEditingMemberID}
        onUpdateDraft={onUpdateFollowupDraft}
        onEditMember={onEditFollowupMember}
        onSave={onSaveFollowup}
        onShowRaw={() => onSetRecordDetailPanel('raw')}
      />
    ),
  })
      ...
      {recordDetailState.loading ? (
        ...
        <div className="asset-decision-detail asset-decision-record-detail">
          ...
          {recordDetailPanel === 'execution' && renderDetailPanel('执行跟进',
            <>
              <form
                className="asset-decision-record-status-form"
                onSubmit={(event) => {
          ...
          {recordDetailPanel === 'members' && renderDetailPanel('成员跟进',
            <>
              <RecordFollowupRows
                members={recordDetailState.detail.members}
                drafts={followupDrafts}
                saving={followupSaving}
                editingMemberID={followupEditingMemberID}
                onUpdateDraft={onUpdateFollowupDraft}
