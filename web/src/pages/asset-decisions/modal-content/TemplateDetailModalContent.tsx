          ...
          {templateDetailPanel === 'status' && !templateDetailState.detail.builtin && renderDetailPanel('状态维护',
            <>
              ...
              <ActionConfirmationModal
                open={pendingTemplateStatus != null}
                title={pendingTemplateStatus === 'archived' ? '确认归档模板' : '确认重新启用模板'}
                current={`模板状态为 ${SCENARIO_TEMPLATE_STATUS_LABELS[templateDetailState.detail.status]}`}
                result={pendingTemplateStatus ? `模板状态变为 ${SCENARIO_TEMPLATE_STATUS_LABELS[pendingTemplateStatus]}` : '模板状态保持不变'}
                impact={pendingTemplateStatus === 'archived'
                  ? '归档后不能直接从该模板创建新组合。'
                  : '重新启用后可继续从该模板创建自定义组合。'}
                unchanged="不会修改已经创建的自定义组合、决策记录或任何 VPS 资产事实。"
                confirmLabel={templateSaving ? '更新中…' : pendingTemplateStatus === 'archived' ? '确认归档模板' : '确认重新启用'}
                disabled={templateSaving}
                onCancel={onCancelTemplateStatusUpdate}
                onConfirm={() => {
                  if (pendingTemplateStatus) onUpdateTemplateStatus(pendingTemplateStatus)
                }}
              />
            </>,
          )}
