...
describe('ProvidersPage', () => {
  ...
  it('renders provider directory context from providers, VPS, and subscriptions', async () => {
    ...
    const directoryTitle = screen.getByRole('heading', { name: '服务商与入口' })
    const tableRegion = screen.getByRole('region', { name: '服务商与入口' })
    const scrollHint = screen.getByText('横向滚动查看完整列')
    expect(directoryTitle).toHaveAttribute('id', 'provider-directory-table-title')
    expect(directoryTitle.closest('section')).not.toHaveClass('page-panel--scroll-x')
    expect(tableRegion).toHaveAttribute('tabindex', '0')
    expect(tableRegion).toHaveAttribute('aria-labelledby', directoryTitle.id)
    expect(tableRegion).toHaveAttribute('aria-describedby', scrollHint.id)
    expect(scrollHint).toHaveAttribute('id', 'provider-directory-table-hint')
    ...
    expect(document.querySelector('col[width="160px"]')).toBeInTheDocument()
    expect(document.querySelector('col[width="122px"]')).toBeInTheDocument()
    expect(document.querySelector('col[width="232px"]')).toBeInTheDocument()
    expect(document.querySelector('col[width="154px"]')).toBeInTheDocument()
