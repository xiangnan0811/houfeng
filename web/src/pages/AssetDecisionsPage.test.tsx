...
describe('AssetDecisionsPage', () => {
  ...
  it('renders a unified decision queue and reloads subscription renewals when the window changes', async () => {
    ...
    expect(screen.getByText(/Asset Ledger 的主工作队列/)).toBeInTheDocument()
    expect(screen.getByText(/按续费窗口、未评估、迁移\/取消、Node 关联数量和订阅证据缺口排序/)).toBeInTheDocument()
    ...
    expect(within(evidenceBoundary).getByText('订阅读取失败不会被当成缺订阅')).toBeInTheDocument()
    expect(within(evidenceBoundary).getByText('Node 信号只显示关联数量，不展示健康/心跳')).toBeInTheDocument()
    expect(within(evidenceBoundary).getByText('Drawer 是处理单台 VPS 续费决策的次级面板')).toBeInTheDocument()
    expect(screen.getByText('窗口只重载续费候选证据；队列仍使用已加载订阅证据和 VPS 决策切片。')).toBeInTheDocument()
    expect(screen.getByText('筛选不会改变排序公式；缺订阅仅在全量订阅证据成功读取后才作为资料缺口显示。')).toBeInTheDocument()
    expect(screen.getByText(/次级证据区只展示续费窗口内的订阅记录/)).toBeInTheDocument()
    expect(screen.getByText('2026-05-20')).toBeInTheDocument()
    expect(screen.getAllByText('按证据排序').length).toBeGreaterThan(0)
  ...
  it('shows next actions when a queue view is empty', async () => {
    ...
    expect(screen.getByText('这组队列暂时没有需要人工决策的资产；可回到全部队列、库存或订阅证据继续核对。若订阅证据曾读取失败，请不要把空视图解读为缺订阅事实。')).toBeInTheDocument()
  ...
  it('shows a queue error instead of misreporting missing subscriptions when all subscription evidence fails', async () => {
    ...
    expect(screen.getByText(/队列证据读取失败时不会把 VPS 渲染成缺订阅/)).toBeInTheDocument()
