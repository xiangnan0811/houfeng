...
describe('AssetDecisionsPage', () => {
  ...
  it('renders a unified decision queue and reloads subscription renewals when the window changes', async () => {
    ...
    expect(screen.getByText('订阅读取失败不会被当成缺订阅')).toBeInTheDocument()
    expect(screen.getByText('Node 信号只显示关联数量，不展示健康/心跳')).toBeInTheDocument()
    expect(screen.getByText('Drawer 是处理单台 VPS 续费决策的次级面板')).toBeInTheDocument()
