...
describe('AssetDecisionsPage', () => {
  ...
  it('opens group detail with member comparison, evidence, and single VPS entry', async () => {
    ...
    expect(within(command).getByText('主力承载明确，备用仍需补齐订阅和监控证据')).toBeInTheDocument()
    expect(within(command).getByRole('button', { name: '创建自定义组合' })).toBeInTheDocument()
    expect(within(command).getByRole('button', { name: '保存为决策记录' })).toBeInTheDocument()
    ...
    expect(within(dialog).getByLabelText('关键成员摘要')).toBeInTheDocument()
  ...
  it('opens saved decision records and patches record status', async () => {
    ...
    expect(within(sourcePanel).getByText(/自动组 · renewal_attention · 续费取舍 · adg_auto_001/)).toBeInTheDocument()
