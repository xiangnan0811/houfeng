import { render, screen } from '@testing-library/react'
import { describe, expect, it } from 'vitest'

import { NodeStatusSummary } from './NodeStatusSummary'
import type { NodeRecord } from '../../lib/types'

function nodeRecord(overrides: Partial<NodeRecord> = {}): NodeRecord {
  return {
    node_id: 'nd_001',
    display_name: 'Tokyo Edge',
    region: 'ap-northeast-1',
    city: 'Tokyo',
    provider: 'Vultr',
    lifecycle_status: '在用',
    monitoring_status: '启用',
    binding_status: '已绑定',
    labels: ['core'],
    note: '',
    current_health_status: '关注',
    last_heartbeat_at: '2026-04-27T09:00:00Z',
    last_sync_at: '2026-04-27T09:05:00Z',
    current_active_incident_count: 2,
    current_primary_issue_summary: '检测到磁盘压力',
    created_at: '2026-04-20T00:00:00Z',
    updated_at: '2026-04-27T09:05:00Z',
    ...overrides,
  }
}

describe('NodeStatusSummary', () => {
  it('renders three summary cards with health, incident count and primary issue', () => {
    render(<NodeStatusSummary node={nodeRecord()} />)

    expect(screen.getByText('健康状态')).toBeInTheDocument()
    expect(screen.getByText('关注')).toBeInTheDocument()
    expect(screen.getByText('活跃异常数')).toBeInTheDocument()
    expect(screen.getByText('2')).toBeInTheDocument()
    expect(screen.getByText('当前主问题')).toBeInTheDocument()
    expect(screen.getByText('检测到磁盘压力')).toBeInTheDocument()
  })

  it('falls back to "暂无明显异常" when there is no primary issue', () => {
    render(<NodeStatusSummary node={nodeRecord({ current_primary_issue_summary: '' })} />)

    expect(screen.getByText('暂无明显异常')).toBeInTheDocument()
  })
})
